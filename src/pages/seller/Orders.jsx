    ...
    <div className='no-scrollbar  flex-1 h-[95vh] overflow-y-scroll '>
      <div className="md:p-10 p-4 space-y-4">
        ...
        {orders.map((order, index) => (
          ...
          rounded-md border text-gray-300">
            <div className="flex gap-5 max-w-80">
              <img className="w-12 h-12 object-cover " src={assets.box_icon} alt="boxIcon" />
              <div >
                {order.items.map((item, index) => (
                  <div key={index} className="flex flex-col ">
                    <p className="font-medium text-black/88">
                      {item.product.name}{" "} <span className='text-green-500/80' >x {item.quantity}</span>
                    </p>
                  </div>
                ))}
              </div>
            ...
            <div className="text-sm md:text-base text-black/60 ">
              <p className='text-black/88'>{order.address.firstName} {order.address.lastName}</p>
