    ...
    return !isSeller && (
        <form onSubmit={onSubmitHandler} className='min-h-screen flex items-center text-sm text-gray-600 '>
         ...
         rounded-lg shadow-xl border border-gray-200 '>
                <p className='text-2xl font-medium m-auto '><span className='text-green-500/80'>Seller </span>Login </p>
                <div className='w-full'>
                    <p>Email</p>
                    <input onChange={(e) => setEmail(e.target.value)} value={email} type="email" placeholder='enter your email'
                        className='border border-gray-200 rounded w-full p-2 mt-1 outline-green-500/80' required />
                </div>
                <div className='w-full'>
                    <p>Password</p>
                ...
                <button className='bg-green-500/80 text-white w-full py-2 rounded-md cursor-pointer '>Login</button>
            </div>
        </form>
    )
}
